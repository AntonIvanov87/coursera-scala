...
case class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    val lmr = NonEmpty.mostRetweeted(left, elem)
    val rmr = NonEmpty.mostRetweeted(right, elem)

    if (lmr.retweets >= elem.retweets && lmr.retweets >= rmr.retweets)
      lmr
    else if (elem.retweets >= lmr.retweets && elem.retweets >= rmr.retweets)
      elem
    else
      rmr
  }
  ...
  def descendingByRetweet: TweetList = {
    ...
    val withoutMr = this.remove(mr)
    new Cons(mr, withoutMr.descendingByRetweet)
  }
...
object NonEmpty {
  ...
  private def mostRetweeted(tweetSet: TweetSet, default: Tweet): Tweet =
    try
      tweetSet.mostRetweeted
    catch {
      case ex: NoSuchElementException => default
    }
...
object GoogleVsApple {
  ...
  private def textContainsOneOf(text: String, oneOf: List[String]): Boolean =
    oneOf.exists((s: String) => text.contains(s))

  private val allTweets = TweetReader.allTweets

  lazy val googleTweets: TweetSet = allTweets.filter(
    (t: Tweet) => textContainsOneOf(t.text, google)
  )
  lazy val appleTweets: TweetSet = allTweets.filter(
    (t: Tweet) => textContainsOneOf(t.text, apple)
  )
